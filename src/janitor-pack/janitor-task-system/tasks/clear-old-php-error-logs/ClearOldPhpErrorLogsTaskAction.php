...
/**
 * Clear Old PHP Error Logs task action
 * ------------------------------------
 *
...
/**
 * Class ClearOldPhpErrorLogsTaskAction
 * @package Pith\Janitor
 */
class ClearOldPhpErrorLogsTaskAction extends PithAction
{
    ...
    public function runAction()
    {
        ...
        $cli_writer->writeLine($format->fg_bright_yellow . '┃  Janitor: Clear Old PHP Error Logs  ┃' . $format->reset);
        $cli_writer->writeLine($format->fg_bright_yellow . '┗━───────────────────────────────────━┛' . $format->reset);
        $cli_writer->writeLine(' ');

        $time_21_days = 1814400; // 21 days in seconds
        $this->janitor_service->clearOldLogs(PITH_PHP_ERROR_LOG_LOCATION, $time_21_days);
    }
