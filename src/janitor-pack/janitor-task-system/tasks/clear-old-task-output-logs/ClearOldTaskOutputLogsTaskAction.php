...
/**
 * Clear Old Task Output Logs task action
 * -------------------------------------
 *
...
/**
 * Class ClearOldTaskOutputLogsTaskAction
 * @package Pith\Janitor
 */
class ClearOldTaskOutputLogsTaskAction extends PithAction
{
    ...
    public function runAction()
    {
        ...
        $cli_writer->writeLine($format->fg_bright_yellow . '┃  Janitor: Clear Old Task Output Logs  ┃' . $format->reset);
        $cli_writer->writeLine($format->fg_bright_yellow . '┗━─────────────────────────────────────━┛' . $format->reset);
        $cli_writer->writeLine(' ');

        $time_21_days = 1814400; // 21 days in seconds
        $this->janitor_service->clearOldLogs(PITH_TASK_OUTPUT_LOG_LOCATION, $time_21_days);
    }
